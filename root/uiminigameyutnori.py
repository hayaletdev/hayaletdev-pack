...
class YutnoriWaitingPage(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			if app.ENABLE_YUTNORI_EVENT_FLAG_RENEWAL:
				...
				self.yut_piece_slot.SetOverInItemEvent(ui.__mem_func__(self.__SlotOverInItem), item.ITEM_VNUM_YUT_PIECE)
				...
				self.yut_board_slot.SetOverInItemEvent(ui.__mem_func__(self.__SlotOverInItem), item.ITEM_VNUM_YUT_BOARD)
			...
			if localeInfo.IsARABIC():
				...
				if app.ENABLE_YUTNORI_EVENT_FLAG_RENEWAL:
					yut_flag_count_window = self.GetChild("yut_flag_count_window")
					(yut_flag_count_window_x, yut_flag_count_window_y) = yut_flag_count_window.GetLocalPosition()
					yut_flag_count_window.SetPosition(yut_flag_count_window_x - 70, yut_flag_count_window_y)
	...
	def __ClickStartButton(self):
		if app.ENABLE_YUTNORI_EVENT_FLAG_RENEWAL:
			if self.yut_board_count == 0 or player.GetElk() < YUTNORI_START_GOLD:
				chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.YUTNORI_EVENT_MESSAGE_NOT_ENOUGHT_ITEM)
				return
	...
	def Show(self):
		...
		event.SetRestrictedCount(self.descIndex, DESC_WIDTH_COUNT) # DESC_WIDTH_COUNT mainline 과 development 가 폰트체가 달라서
	...
	if app.ENABLE_YUTNORI_EVENT_FLAG_RENEWAL:
		...
		def SetItemToolTip(self, tooltip):
			self.tooltip_item = proxy(tooltip)
...
class YutnoriGamePage(ui.ScriptWindow):
	def __init__(self):
		...
		self.prob_name_tuple = (uiScriptLocale.MINI_GAME_YUTNORI_YUTSEM1
	...
	def __ClickProbButtonOver(self, index):
		...
		if localeInfo.IsARABIC():
			prob_select_over_img_width = self.prob_select_over_img.GetWidth()
			self.prob_select_over_img.SetPosition(328 + prob_select_over_img_width, 168 + 16 * index)
		else:
	...
	def __UpdateAlpha(self):
		...
		if self.yut_cur_alpha >= 1.0:
			...
			if self.yutnori_state in [ YUTNORI_BEFORE_TURN_SELECT, YUTNORI_AFTER_TURN_SELECT ]:
				...
				if self.notice_text:
					self.notice_text.SetText(localeInfo.MINI_GAME_NOTICE_8 % self.prob_name_tuple[yut_result]) # %s 나왔습니다!

			elif True == self.pc_turn:
				move_count = self.__GetMoveCount(self.yut_result_pc)
				notice_str = None
				if move_count > 0:
					notice_str = localeInfo.MINI_GAME_NOTICE_6 % (self.prob_name_tuple[self.yut_result_pc], move_count) # %s 나왔습니다! %d칸 이동할 수 있습니다.
				else:
					notice_str = localeInfo.MINI_GAME_NOTICE_7 % self.prob_name_tuple[self.yut_result_pc] # %s 나왔습니다! 1칸 뒤로 이동 해야합니다.
