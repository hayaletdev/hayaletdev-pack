...
class AutoBanQuizWindow(ui.ScriptWindow):
	...
	def LoadWindow(self):
		try:
			...
			pyScrLoader.LoadScriptFile(self, uiScriptLocale.LOCALE_COMMON_UISCRIPT_PATH + "AutoBanQuiz.py")
	...
	def __OnClickAnswerButton(self):
		if self.answer:
			...
			net.SendCommandPacket("/autoban_answer %d" % (self.answer))
			self.Close()
		else:
	...
	def __OnClickRefreshButton(self):
		...
		net.SendCommandPacket("/autoban_refresh")
