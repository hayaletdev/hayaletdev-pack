...
WORD_MAX = 0xffff
MAILBOX_MAX_DISTANCE = 1000 # 우편함 npc 와의 체크 거리(아이템을 사용하지 않았다면 이 거리 이상 멀어지게 되면 창이 꺼짐)

# 우편함 편지 구분
DIVISION_EMPTY = 0 # 빈 우편
DIVISION_POST = 1 # 일반 우편
DIVISION_ITEM = 2 # 아이템 or won or yang 이 첨부된 우편
DIVISION_GM = 3 # GM 이 보낸 우편

POST_MAX = 9 # 한번에 노출될 우편 MAX
TITLE_MAX_LEN = 25 # 우편함 클릭시 제목 MAX LEN

POST_WRITE_LIMIT_LEVEL = 20 # 발송하기 레벨 제한
FRIEND_SELECT_UI_SHOW_MAX = 10 # 친구 리스트 SHOW MAX

MAX_YANG = 9998999 # 보낼 수 있는 yang MAX
MAX_CHEQUE = 99 # 보낼 수 있는 won MAX
POST_WRITE_COST = 1000 # 우편 보낼때 소비 비용
		...
		if None == item_data:
			...
			if app.ENABLE_CHANGE_LOOK_SYSTEM:
				...
				if not changelookvnum == 0:
					self.item_slot.SetSlotCoverImage(0, "icon/item/ingame_convert_Mark.tga")
				else:
					self.item_slot.EnableSlotCoverImage(0, False)
			self.item_slot.RefreshSlot()
...
class PostWrite(ui.ScriptWindow):
	...
	def SelectEmptySlot(self, selectedSlotPos):
		...
		if app.ENABLE_CHANGE_LOOK_SYSTEM:
			if not player.GetChangeLookVnum(window_type, attachedSlotPos) == 0:
				self.item_slot.SetSlotCoverImage(0, "icon/item/ingame_convert_Mark.tga")
			else:
				self.item_slot.EnableSlotCoverImage(0, False)
		self.item_slot.RefreshSlot()
...
class MailBox(ui.ScriptWindow):
	...
	# 모두 삭제 버튼의 기능은 물푼으로 온 우편이 아닌 단순 우편만 삭제할 수 있는 버튼
	...
	def __ClickPost(self, button_index, data_index):
		...
		## 추가 data 가 없다면 서버로부터 정보를 요청해야 함.
	...
	def __ClickNextButton(self):
		...
		max_page = int(math.ceil(len(self.post_data_dict) / float(POST_MAX))) # 올림처리
	...
	def __PostGetItemsFail(self, result):
		if mail.POST_GET_ITEMS_FAIL == result:
			...
			pass
			#print "차단한 사람의 우편은 받을 수 없다."
			#print "내가 보낸사람 아이디를 차단한 경우 받을 수 없다."
