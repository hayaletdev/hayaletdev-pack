...
class DeclareGuildWarDialog(ui.ScriptWindow):
	...
	def __CreateDialog(self):
		try:
			...
			import exception
			exception.Abort("DeclareGuildWarDialog.__CreateDialog - LoadScript")
		...
		try:
			...
			import exception
			exception.Abort("DeclareGuildWarDialog.__CreateDialog - BindObject")
	...
	def __OnOK(self):
		...
		net.SendCommandPacket("/war %s %d" % (text, type))
		self.Close()
...
class AcceptGuildWarDialog(ui.ScriptWindow):
	...
	def __CreateDialog(self):
		try:
			...
			import exception
			exception.Abort("AcceptGuildWarDialog.__CreateDialog - LoadScript")
		...
		try:
			...
			import exception
			exception.Abort("AcceptGuildWarDialog.__CreateDialog - BindObject")
...
class GuildWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			if localeInfo.IsARABIC():
				...
				self.pageWindow = {
					"GUILD_INFO" : self.PageWindow(self, uiScriptLocale.LOCALE_COMMON_UISCRIPT_PATH + "GuildWindow_GuildInfoPage.py"),
					"BOARD" : self.PageWindow(self, uiScriptLocale.LOCALE_COMMON_UISCRIPT_PATH + "GuildWindow_BoardPage.py"),
					"MEMBER" : self.PageWindow(self, uiScriptLocale.LOCALE_COMMON_UISCRIPT_PATH + "GuildWindow_MemberPage.py"),
					"BASE_INFO" : self.PageWindow(self, "UIScript/GuildWindow_BaseInfoPage.py"),
	...
	def __MakeMemberPage(self):
		...
		for i in xrange(self.MEMBER_LINE_COUNT):
			...
			if localeInfo.IsJAPAN():
				...
				jobSlotImage = ui.MakeImageBox(page, "d:/ymir work/ui/public/Parameter_Slot_00.sub", 152, yPos)
			else:
			...
			if localeInfo.IsJAPAN():
				...
				levelSlotImage = ui.MakeImageBox(page, "d:/ymir work/ui/public/Parameter_Slot_00.sub", 112, yPos)
			else:
			...
			if localeInfo.IsJAPAN():
				...
				offerSlotImage = ui.MakeImageBox(page, "d:/ymir work/ui/public/Parameter_Slot_00.sub", 72, yPos)
			else:
	...
	def OnUpGuildSkill(self, slotNumber, type):
		skillIndex = player.GetSkillIndex(slotNumber)
		net.SendCommandPacket("/gskillup " + str(skillIndex))
...
class BuildGuildBuildingWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			if localeInfo.IsARABIC():
				self.categoryList.SetPosition(0, 1)
				self.categoryList.SetWindowHorizontalAlignCenter()
		except:
			import exception
	...
	if app.ENABLE_10TH_EVENT:
		...
		def __AskExchangeTenth(self):
			...
			net.SendCommandPacket("/build c %d %d %d %d %d %d" % (self.race, int(self.x), int(self.y), self.rot_x, self.rot_y, self.rot_z))
			self.Close()
	"""
	...
	def Build(self):
		...
		if "BUILDIN" == self.type:
			for i in xrange(len(self.raceList)):
				...
				rot = self.rotList[i]
				net.SendCommandPacket("/build c %d %d %d %d %d %d" % (race, int(self.x+xPos), int(self.y+yPos), self.rot_x, self.rot_y, rot))
		else:
			net.SendCommandPacket("/build c %d %d %d %d %d %d" % (self.race, int(self.x), int(self.y), self.rot_x, self.rot_y, self.rot_z))
