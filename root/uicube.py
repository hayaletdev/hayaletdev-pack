...
class CubeWindow(ui.ScriptWindow):
						...
						for slotPos in xrange(self.cubeSlot.GetSlotCount()):
							...
							if not slotPos in self.cubeItemInfo:
								...
								if True == bCanAddSlot:
									...
									net.SendCommandPacket("/cube add %d %d" % (slotPos, i))
	...
	def __OnCloseButtonClick(self):
		if self.isUsable:
			...
			#print "큐브 닫기"
			net.SendCommandPacket("/cube close")
	...
	def __OnAcceptButtonClick(self):
		...
		#	net.SendCommandPacket("/cube add " + str(invenPos))
		net.SendCommandPacket("/cube make")
		...
		if isAttached:
			...
			if app.ENABLE_EXTEND_INVEN_SYSTEM:
				...
				if app.ENABLE_SOULBIND_SYSTEM:
					if player.GetItemSealDate(window, attachedSlotPos) != item.E_SEAL_DATE_DEFAULT_TIMESTAMP:
						return
				...
				net.SendCommandPacket("/cube add %d %d" % (selectedSlotPos, attachedSlotPos))
			else:
	...
	def __OnSelectItemSlot(self, selectedSlotPos):
		...
		if isAttached:
			...
			net.SendCommandPacket("/cube del %d " % selectedSlotPos)
