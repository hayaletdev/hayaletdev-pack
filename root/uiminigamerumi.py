...
LOW_TOTAL_SCORE = 300
MID_TOTAL_SCORE = 400
LOW_TOTAL_SCORE_NORMAL = LOW_TOTAL_SCORE
...
class RumiWaitingPage(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			if app.ENABLE_OKEY_EVENT_FLAG_RENEWAL:
				...
				self.rumi_card_piece_slot.SetOverInItemEvent(ui.__mem_func__(self.__SlotOverInItem), item.ITEM_VNUM_RUMI_CARD_PIECE)
				...
				self.rumi_card_piece_count_text.SetText("%d/%d" % (0, RUMI_CARD_PIECE_COUNT_MAX))

				self.rumi_card_slot = self.GetChild("rumi_card_slot")
				self.rumi_card_slot.SetOverInItemEvent(ui.__mem_func__(self.__SlotOverInItem), item.ITEM_VNUM_RUMI_CARD_PACK)
				...
				self.rumi_card_count_text.SetText("%d/%d" % (0, RUMI_CARD_COUNT_MAX))
			...
			if localeInfo.IsARABIC():
				...
				if app.ENABLE_OKEY_EVENT_FLAG_RENEWAL:
					slot_window = self.GetChild("slot_window")
					(slot_window_x, slot_window_y) = slot_window.GetLocalPosition()
					slot_window.SetPosition(slot_window_x - 50, slot_window_y)
		except:
			import exception
	...
	def __ClickStartButton(self):
		if app.ENABLE_OKEY_EVENT_FLAG_RENEWAL:
			if self.rumi_card_count == 0 or player.GetElk() < RUMI_START_GOLD:
				chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.OKEY_EVENT_MESSAGE_NOT_ENOUGHT_ITEM)
				return

		if None == self.start_question_dialog:
			self.start_question_dialog = uiCommon.QuestionDialog()
			self.start_question_dialog.SetText(localeInfo.MINI_GAME_RUMI_START_QUESTION % (RUMI_START_GOLD, 1))
	...
	def Show(self):
		...
		event.ClearEventSet(self.desc_index)
		self.desc_index = event.RegisterEventSet(uiScriptLocale.MINIGAME_RUMI_DESC)
		...
		event.SetRestrictedCount(self.desc_index, DESC_WIDTH_COUNT) # DESC_WIDTH_COUNT mainline 과 development 가 폰트체가 달라서
		...
		if app.ENABLE_OKEY_EVENT_FLAG_RENEWAL:
			try:
				item.SelectItem(item.ITEM_VNUM_RUMI_CARD_PIECE)
				self.rumi_card_piece_slot.SetSlot(0, 0, 32, 32, item.GetIconImage(), (1.0, 1.0, 1.0, 1.0))

				item.SelectItem(item.ITEM_VNUM_RUMI_CARD_PACK)
				self.rumi_card_slot.SetSlot(0, 0, 32, 32, item.GetIconImage(), (1.0, 1.0, 1.0, 1.0))
			except:
				import exception
				exception.Abort("MiniGameRumiWaitingPage.Show.SelectItem")
	...
	if app.ENABLE_OKEY_EVENT_FLAG_RENEWAL:
		...
		def MiniGameRumiFlagProcess(self, process_type, data):
			(rumi_card_piece_count, rumi_card_count) = data

			self.rumi_card_piece_count = rumi_card_piece_count
			self.rumi_card_count = rumi_card_count
...
class RumiGamePage(ui.ScriptWindow):
	...
	def __ShowDeckSlotBaseImage(self, index):
		deck_slot_cnt = self.card_slot[DECK_CARD].GetSlotCount()
		for i in range(0, deck_slot_cnt):
			if i == index:
...
class MiniGameRumi(ui.Window):
	...
	def GameStart(self):
		if self.cur_page:
			if app.ENABLE_OKEY_EVENT_FLAG_RENEWAL:
				self.cur_page.DecreaseMiniGameRumiCardCount()
			self.cur_page.Close()
