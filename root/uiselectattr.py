...
class SelectAttrWindow(ui.ScriptWindow):
	...
	def Open(self, window_type, slotIdx):
		...
		try:
			...
			Slot.SetItemSlot(0, player.GetItemIndex(self.window_type, self.slotIdx))
			Slot.RefreshSlot()
		...
		## 아이템 속성 가져오기
		CurAttr = [player.GetItemAttribute(self.window_type, self.slotIdx, i) for i in xrange(player.ATTRIBUTE_SLOT_MAX_NUM)]
	...
	def Close(self):
		if self.SelectIdx < 0:
			chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.SELECT_ATTR_NOT_SELECT)
			return
		else:
			net.SendChagedItemAttributePacket(self.SelectIdx, self.window_type, self.slotIdx)
	...
	def OnUpdate(self):
		...
		if app.ENABLE_EXTEND_INVEN_SYSTEM:
			if 0 <= self.slotIdx < INVENTORY_PAGE_SIZE * player.INVENTORY_PAGE_COUNT:
