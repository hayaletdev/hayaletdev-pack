...
class CMouseController(object):
	...
	def Create(self):
		...
		if app.ENABLE_FISHING_GAME:
			self.cursorDict.update({ app.FISH : CursorImage("D:/Ymir Work/UI/Cursor/fishing_mouse_cursor.sub"), })
		...
		if app.ENABLE_FISHING_GAME:
			self.cursorPosDict.update({ app.FISH : (0, 0), })
	...
	def AttachObject(self, Owner, Type, SlotNumber, ItemIndex, count = 0):
		...
		try:
			...
			if Type == player.SLOT_TYPE_INVENTORY or\
				...
				if app.ENABLE_EXTEND_INVEN_SYSTEM and (quickSlotType == player.SLOT_TYPE_INVENTORY or quickSlotType == player.SLOT_TYPE_BELT_INVENTORY):
					window = player.SlotTypeToInvenType(quickSlotType)
					itemIndex = player.GetItemIndex(window, position)
					item.SelectItem(itemIndex)
					self.AttachedIconHandle = item.GetIconInstance()
					(width, height) = item.GetItemSize()

				if not app.ENABLE_EXTEND_INVEN_SYSTEM and quickSlotType == player.SLOT_TYPE_INVENTORY:
				...
				if Type == player.SLOT_TYPE_EQUIPMENT or\
					Type == player.SLOT_TYPE_BELT_INVENTORY:
