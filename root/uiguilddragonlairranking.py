...
class GuildDragonLairRankingDialog(ui.ScriptWindow):
	...
	def RefreshGuildDragonLairRanking(self, type):
		...
		for line, ResultSlotList in self.ResultSlotList.items():
			...
			if nowIndex >= guild.GetDragonLairRankingShowCount(type):
				break

			if type == guild.GUILD_DRAGONLAIR_TYPE_RED:
				(guildname, membercount, min, sec) = guild.GetDragonLairRanking(type, nowIndex)
		...
		if not app.ENABLE_GUILD_DRAGONLAIR_PARTY_SYSTEM:
			## 스크롤바 다시 세팅
			self.ScrollBar.SetMiddleBarSize(float(self.MAX_LINE_COUNT) / float(self.CheckNowItemCount(type)))
	...
	def CheckNowItemCount(self, type):
		if guild.GetDragonLairRankingShowCount(type) <= self.MAX_LINE_COUNT:
			return self.MAX_LINE_COUNT
		else:
			return guild.GetDragonLairRankingShowCount(type)

	def OnScrollControl(self):
		nowItemCount = 0
		if guild.GetDragonLairRankingShowCount(self.DragonLairType) <= self.MAX_LINE_COUNT:
			nowItemCount = 0
		else:
			nowItemCount = (guild.GetDragonLairRankingShowCount(self.DragonLairType) - self.MAX_LINE_COUNT)
